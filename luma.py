...
def _enrich_from_detail(ev: Event, timeout: float) -> None:
    ...
    for finder in (
        ...
        if tag and tag.get("content"):
            ev.description = ev.description or tag["content"][:1500]
            break

    # fallback: nejdelsi textovy blok na strance (popis eventu byva nejdelsi)
    if not ev.description:
        blocks = [t.get_text(" ", strip=True) for t in soup.find_all(["div", "section", "p"])]
        longest = max((b for b in blocks if 80 < len(b) < 4000), key=len, default="")
        if longest:
            ev.description = re.sub(r"\s+", " ", longest)[:1500]

    # datum z <time datetime="...">
    if not ev.date:
        t = soup.find("time", attrs={"datetime": True})
        if t:
            ev.date = t["datetime"][:10]
